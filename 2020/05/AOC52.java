...
public class AOC52 {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            while ((trenutnaVrstica = bralec.readLine()) != null)   {
                ...
                if (maxSedez == null) {
                    ...
                    ids.add(maxSedez.getId());
                } else {
                    Seat trenutniSedez = new Seat(trenutnaVrstica);
                    ids.add(trenutniSedez.getId());
        ...
        // od tu naprej lahko predpostavim, da imam v spremnljivki 
        // `ids` vse id-je sedezev, v spremenljivki `maxSeat` pa 
        // id zadnjega sedeza oz. kar stevilo vseh sedezev

        boolean[] zasedeni = new boolean[maxSeat + 1];

        for (int i = 0; i < ids.size(); i++) {
            int idTrenutnoGledanegaSedeza = ids.get(i);
            if (zasedeni[idTrenutnoGledanegaSedeza]) {
                System.out.println("Duplicate!");
                System.exit(1);
            }
            zasedeni[idTrenutnoGledanegaSedeza] = true;
        }

        for (int i = 0; i < zasedeni.length; i++) {
            if (i > 0 && zasedeni[i - 1] 
                && zasedeni[i] == false 
                && i < zasedeni.length && zasedeni[i + 1]) {
